import React from 'react';
import { motion } from 'framer-motion';

// Extract data to reuse in both Mobile (List) and Desktop (Grid) views
const featuresData = [
  {
    title: "Tutor-led sessions.",
    img: "https://www.dell.org/wp-content/uploads/2024/05/india-schools-featured-v2.webp"
  },
  {
    title: "Flexible options.",
    img: "https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=400&q=80"
  },
  {
    title: "Online portal access for materials.",
    img: "https://images.unsplash.com/photo-1501504905252-473c47e087f8?w=600&q=80"
  },
  {
    title: "Practice assignments after sessions.",
    img: "https://images.unsplash.com/photo-1434030216411-0b793f4b4173?w=600&q=80"
  },
  {
    title: "Practice assignments",
    img: "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=800&q=80"
  },
  {
    title: "Graded feedback.",
    img: "https://images.unsplash.com/photo-1519389950473-47ba0277781c?w=400&q=80"
  },
  {
    title: "Regular updates for parents.",
    img: "https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?w=600&q=80"
  }
];

const FeaturesSection = () => {
  const cardVariants = {
    hidden: { opacity: 0, y: 20 },
    visible: (i) => ({
      opacity: 1,
      y: 0,
      transition: {
        delay: i * 0.1,
        duration: 0.5,
        ease: "easeOut"
      }
    })
  };

  const FeatureCard = ({ title, img, className, index }) => (
    <motion.div
      custom={index}
      initial="hidden"
      whileInView="visible"
      viewport={{ once: true }}
      variants={cardVariants}
      whileHover={{ y: -5 }}
      // Added active:scale-95 for touch feedback on mobile
      className={`relative overflow-hidden rounded-xl shadow-md group cursor-pointer active:scale-95 md:active:scale-100 transition-all ${className}`}
    >
      <img 
        src={img} 
        alt={title} 
        className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"
      />
      <div className="absolute inset-0 bg-slate-900/40 group-hover:bg-slate-900/20 transition-colors duration-300" />
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent" />
      
      <div className="absolute bottom-0 left-0 p-5 w-full">
        <p className="text-white font-bold text-lg leading-tight drop-shadow-lg">
          {title}
        </p>
      </div>
    </motion.div>
  );

  return (
    <section className="max-w-7xl mx-auto px-4 py-12 bg-gray-50 overflow-hidden">
      <motion.h2 
        initial={{ opacity: 0, y: -10 }}
        whileInView={{ opacity: 1, y: 0 }}
        className="text-3xl font-black text-center mb-10 text-gray-800 tracking-tight"
      >
        What sets us apart
      </motion.h2>

      {/* ================= MOBILE LAYOUT (Horizontal Scroll) ================= */}
      {/* Visible only on small screens (md:hidden) */}
      <div className="flex md:hidden overflow-x-auto snap-x snap-mandatory gap-4 pb-6 -mx-4 px-4 scrollbar-hide">
        {featuresData.map((feature, index) => (
          <FeatureCard 
            key={index}
            index={index}
            title={feature.title}
            img={feature.img}
            // Fixed width for mobile cards to ensure they fit the screen width nicely
            className="min-w-[85vw] h-[350px] snap-center flex-shrink-0"
          />
        ))}
        {/* Spacer to allow scrolling the last item fully into view */}
        <div className="w-2 shrink-0" />
      </div>

      {/* ================= DESKTOP LAYOUT (Bento Grid) ================= */}
      {/* Hidden on mobile (hidden), visible on medium screens and up (md:grid) */}
      <div className="hidden md:grid grid-cols-4 gap-4 h-[650px]">
        
        {/* Left Column: 2 Small Top, 1 Big Bottom */}
        <div className="col-span-1 grid grid-cols-1 grid-rows-2 gap-4">
            <div className="grid grid-cols-2 gap-4 row-span-1">
                <FeatureCard 
                    index={0}
                    {...featuresData[0]}
                    className="h-full"
                />
                <FeatureCard 
                    index={1}
                    {...featuresData[1]}
                    className="h-full"
                />
            </div>
            <FeatureCard 
                index={2}
                {...featuresData[2]}
                className="row-span-1"
            />
        </div>

        {/* Center Column: Tall Card */}
        <div className="col-span-1">
            <FeatureCard 
                index={3}
                {...featuresData[3]}
                className="h-full"
            />
        </div>

        {/* Right Column: Split into Top Big, Bottom 2 Small */}
        <div className="col-span-2 grid grid-rows-2 gap-4">
            <FeatureCard 
                index={4}
                {...featuresData[4]}
                className="row-span-1"
            />
            <div className="grid grid-cols-2 gap-4 row-span-1">
                <FeatureCard 
                    index={5}
                    {...featuresData[5]}
                    className="h-full"
                />
                <FeatureCard 
                    index={6}
                    {...featuresData[6]}
                    className="h-full"
                />
            </div>
        </div>

      </div>
    </section>
  );
};

export default FeaturesSection;