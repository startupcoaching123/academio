  ...
  const editor = useEditor({
    ...
    editorProps: {
      ...
      transformPastedHTML(html) {
        ...
        // Remove Microsoft Word/Google Docs specific tags and styles
        cleaned = cleaned.replace(/<meta[^>]*>/g, '');
        cleaned = cleaned.replace(/<style[^>]*>[\s\S]*?<\/style>/gi, '');
        cleaned = cleaned.replace(/<script[^>]*>[\s\S]*?<\/script>/gi, '');
        cleaned = cleaned.replace(/<!--[\s\S]*?-->/g, '');

        // Remove all inline styles and classes from all elements
        cleaned = cleaned.replace(/\s+style="[^"]*"/gi, '');
        cleaned = cleaned.replace(/\s+class="[^"]*"/gi, '');

        // Clean up Microsoft Office tags
        ...
        // Remove Word-specific paragraph and div tags
        ...
        // Remove font tags and their content
        cleaned = cleaned.replace(/<\/?font[^>]*>/gi, '');

        // Fix excessive spacing - replace multiple <br> tags and empty paragraphs
        ...
        // Fix bullet lists - ensure proper structure
        ...
        // Clean up bold and italic tags - only convert when necessary
        cleaned = cleaned.replace(/<b(\s[^>]*)?>(.*?)<\/b>/gi, '<strong>$2</strong>');
        cleaned = cleaned.replace(/<strong[^>]*>(.*?)<\/strong>/gi, (match, content) => {
          // Only keep strong tags if they were originally bold
          return content.trim() ? `<strong>${content}</strong>` : content;
        });

        cleaned = cleaned.replace(/<i(\s[^>]*)?>(.*?)<\/i>/gi, '<em>$2</em>');

        // Remove any remaining style and class attributes
        cleaned = cleaned.replace(/\s+style="[^"]*"/gi, '');
        cleaned = cleaned.replace(/\s+class="[^"]*"/gi, '');
        ...
        // Fix paragraph spacing - ensure consistent single spacing
